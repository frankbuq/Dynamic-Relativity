...
\author{\textbf{[Frank Buquicchio]} \\ Independent Researcher \\ \href{mailto:frankbuq@gmail.com}{frankbuq@gmail.com}}
\date{\today}
...
% --- ABSTRACT (Auto-centered and narrowed by LaTeX) ---
\begin{abstract}
	...
	We explicitly derive the \textbf{Dual-Action Mechanism}, demonstrating how the Lorentzian signature of spacetime allows local attractive gravity (subluminal) and global repulsive gravity (superluminal) to coexist in the same metric volume. We report the \textbf{empirical detection} of the predicted mass-dependent saturation horizon in Gaia DR3 data, where wide binaries deviate from Newtonian dynamics at $\approx 2,500$ AU. Finally, we present a quantitative residual analysis of GW150914, revealing anomalous damping consistent with vacuum relaxation.
\end{abstract}

% --- CO-CREATION STATEMENT (Forced Center Alignment) ---
\vspace{1em}
\begin{center} % <--- Forces the box to sit in the exact middle
	\fbox{%
		\parbox{0.9\textwidth}{% <--- Sets box width to 90% of page text
...
This persistent failure suggests that the issue may not be a missing particle, but a missing geometric principle. Standard GR fundamentally relies on the \textbf{Rigid Measure Hypothesis}: the assumption that the ``stiffness'' of the metric—its resistance to deformation—is a universal invariant ($c^4/8\pi G$), independent of the kinematic state or field strength of the source.
...
While the concept of velocity-dependent gravitational interactions has historically been explored—ranging from Gerber's early potential models \cite{gerber1898ausbreitung} to modern treatments of dynamical friction in MOND \cite{sanchez2009drag}—these approaches typically treat motion as a perturbation of the potential field itself. Similarly, non-local gravity theories \cite{mashhoon2008nonlocal} introduce memory effects through constitutive relations. Dynamic Relativity differs fundamentally from these precedents by identifying the mechanism not as a field perturbation, but as a \textit{geometric phase transition} in the vacuum stiffness. 
\newpage	%
% -------------------------------------------------------------------
	...
	This scalar field $\mu$ acts as a \textbf{Variable Dielectric of Spacetime}. Just as a dielectric material polarizes in response to an electric field, the geometry of spacetime ``saturates'' in response to a gravitational gradient.
	\begin{itemize}
	...
	This introduces the phenomenon of \textbf{Metric Hysteresis}. The geometry of spacetime possesses a ``memory'' of the source's path. The potential at a given point depends on the \textbf{Retarded Position} of the mass. This is mathematically analogous to the Lienard-Wiechert potentials in electrodynamics, but applied to the stiffness of the manifold itself.
	...
	The phenomenological success of Dynamic Relativity relies on a \textbf{Geometric Phase Transition} that occurs at the critical acceleration scale $a_0$. This transition can be understood as a change in the effective topology of the gravitational flux lines.
	...
	\end{equation}
	This $1/r$ decay naturally produces \textbf{Flat Rotation Curves} without the need for a Dark Matter halo. The "missing mass" is simply a "missing dimension" of flux spreading; the gravity is not spreading vertically, so it remains stronger at large distances.
	...
	To bridge the Riemannian (Newtonian) and Weyl (Galactic) regimes continuously, we introduce a \textbf{Saturation Function} $\mathcal{F}(X)$ that governs the coupling strength between the matter source and the metric deformation. Let $X$ be the dimensionless field strength invariant, defined as the square of the gradient norm scaled by the critical acceleration $a_0$:
	\begin{equation}
	...
	\end{equation}
	This equation embodies the central tenet of the theory: \textbf{Stress induces Rigidity.}
	\begin{itemize}
	...
	For complex multi-body systems, a scalar factor is insufficient to capture the directional nature of the stress. We elevate the scalar factor to a rank-2 \textbf{Disformal Tensor} $\tilde{g}_{\mu\nu}$. The physical metric is constructed by deforming the background metric along the direction of the gradient:
	\begin{equation}
	...
	\end{align}
	A real gradient norm produces a positive saturation in the hyperbolic function, pulling the metric towards rigidity. In the weak-field limit, a positive scalar coupling manifests as \textbf{Newtonian Attraction}.
	...
	\subsection{The Spectral Interpretation: Gravitational Impedance}
	To intuitively visualize how the vacuum can exhibit both attractive (Newtonian/Galactic) and repulsive (Cosmological) behaviors simultaneously, it is useful to adopt a spectral metaphor. We may treat the vacuum manifold not as a static stage, but as a \textbf{Dispersive Medium} with a frequency-dependent impedance.
	
	In this analogy, the ``frequency'' of the interaction is defined by the gradient intensity of the potential, $\omega \sim |\nabla \Phi|$. The source mass acts as a driver, and the scalar field $\mu$ represents the response function (susceptibility) of the medium.
	
	\subsubsection{1. Attraction vs. Repulsion: The Phase Response}
	Just as a driven harmonic oscillator responds differently depending on whether it is driven below or above its resonant frequency, the vacuum geometry responds differently to subluminal and superluminal sources.
	\begin{itemize}
		\item \textbf{Subluminal Regime ($v < c$, In-Phase):} Local matter moves slower than the propagation speed of the metric field. The vacuum response is \textit{in-phase} with the source mass. The medium stiffens positively ($\mu \ge 1$), creating a potential well. This manifests as \textbf{Attraction}.
		\item \textbf{Superluminal Regime ($v > c$, Out-of-Phase):} The cosmological horizon recedes faster than $c$. The driver is effectively "super-resonant." The response of the medium lags by $180^\circ$ (represented mathematically by the imaginary transition $\mathcal{S} \to i\mathcal{S}$). This phase inversion turns the potential well into a potential hill. This manifests as \textbf{Repulsion} (Dark Energy).
	\end{itemize}
	
	\subsubsection{2. Variable Strength: Frequency Detuning (The EFE)}
	Crucially, this framework explains why gravity's strength is not fixed, but depends on the environment (e.g., the External Field Effect).
	Consider two interacting bodies, A and B. Their mutual gravitational strength depends on whether their interaction ``frequency'' falls within the vacuum's resonant bandwidth.
	\begin{itemize}
		\item \textbf{Isolated System (Resonant):} If A and B are isolated in a void, their gradient is low ($\omega < \omega_{cutoff}$). They tune into the vacuum's ``elastic mode.'' The scalar field resonates ($\mu > 1$), amplifying their gravity. They behave as if they contain Dark Matter.
		\item \textbf{Embedded System (Detuned):} If the same binary pair is placed near a massive host galaxy, the external field imposes a high background gradient (a ``blue-shift''). This shifts the system's effective frequency above the cutoff ($\omega_{total} > \omega_{cutoff}$). The vacuum saturates ($\mu \to 1$), damping the resonance. The system returns to standard Newtonian behavior.
	\end{itemize}
	
	Thus, gravity is not a static force, but a \textbf{context-dependent harmonic interaction}. The ``Dark Matter'' effect is simply the ringing of the vacuum in a quiet environment, which is silenced in a noisy (high-gradient) environment.
	% -------------------------------------------------------------------
	...
	To test this prediction, we performed a targeted analysis of the Gaia Data Release 3 (DR3) catalog \cite{gaiadr3}. We queried for wide binary systems composed strictly of Red Dwarfs ($BP-RP > 1.5$, $M_G > 10$) with high-quality astrometry ($Plx/\sigma > 10$) and separations between 500 AU and 20,000 AU. This "Sniper Query" yielded 6,832 valid systems.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1.0\textwidth]{./gf/gaia_verification_plot.png} 
	...
	In contrast to the Bullet Cluster, Abell 520 exhibits a ``Dark Core'' coinciding with the gas, which contradicts collisionless Cold Dark Matter (CDM) models. In CDM simulations, the dark matter should always separate from the gas. DR explains this via \textbf{Velocity-Dependent Saturation}.
	\begin{itemize}
	...
	Observations indicate that regions of high entropy (hot, shocked gas) exhibit weaker lensing than cold, ordered regions. This is a direct consequence of \textbf{Thermal Saturation}.
	\begin{itemize}
	...
	The discovery of galaxies appearing to lack Dark Matter (NGC 1052-DF2 and DF4) challenged many theories. However, these galaxies are almost always found in the vicinity of a massive host galaxy. This is explained by the \textbf{External Field Effect (EFE)}.
	\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{Subluminal ($v < c$):} The term under the radical is positive. The norm $\mathcal{S}$ is real. The conformal factor $\mu = 1 + a_0/\mathcal{S}$ is positive. Gravity is \textbf{Attractive}.
	...
	\end{equation}
	Physically, this imaginary coupling represents a \textbf{$180^\circ$ Phase Lag} in the metric response. The conformal scaling flips sign. The vacuum stiffness becomes negative, manifesting as a repulsive pressure.
	
	\subsection{Origin of Dark Energy ($\Lambda$)}
	The "Dark Energy" density $\rho_{\Lambda}$ is not a property of empty space, but the aggregate effect of the \textbf{Superluminal Horizon}.
	...
	\subsection{Cosmological Redshift and the Hubble Tension}
	A critical challenge for modern cosmology is the \textbf{Hubble Tension}—the statistically significant ($5\sigma$) discrepancy between the expansion rate $H_0$ measured locally via Cepheids/Supernovae ($H_0 \approx 73$ km/s/Mpc) and that inferred from the Cosmic Microwave Background ($H_0 \approx 67$ km/s/Mpc). $\Lambda$CDM offers no intrinsic mechanism to resolve this.
	
	Dynamic Relativity proposes that this tension is an artifact of the \textbf{Rigid Measure Hypothesis}. Standard cosmology assumes the metric scaling is invariant from the recombination epoch ($z \sim 1100$) to the present day ($z \sim 0$). However, our Master Equation dictates that the vacuum stiffness depends on the clustering of matter.
	
	\subsubsection{1. The Gradient Evolution}
	\begin{itemize}
		\item \textbf{Early Universe (High $z$):} The universe was a nearly homogeneous plasma. The local gradients $|\nabla \Phi|$ were minimal. The vacuum was globally in the \textbf{Relaxed Phase} ($\mu_{CMB} > 1$).
		\item \textbf{Late Universe (Low $z$):} Structure formation has created dense galaxies and clusters. The local universe is dominated by steep potential wells. The vacuum is locally in the \textbf{Saturated Phase} ($\mu_{local} \to 1$).
	\end{itemize}
	
	\subsubsection{2. The Calibration Correction}
	Distance measurements using "Standard Candles" (Type Ia Supernovae) rely on the luminosity distance $D_L$. In Dynamic Relativity, the observed flux $F$ is modulated by the conformal factor $\mu$ of the source environment:
	\begin{equation}
		D_L^{obs} = D_L^{true} \sqrt{\mu(z)}
	\end{equation}
	Because we calibrate our local rung of the distance ladder (Cepheids) in the Milky Way—a saturated, high-gradient environment where $\mu \approx 1$—we systematically underestimate the intrinsic luminosity of sources in less structured, higher-$z$ environments where $\mu > 1$.
	
	When this conformal correction is applied to the Planck data, the inferred value of $H_0$ shifts upward. Dynamic Relativity suggests that the "tension" is simply a measure of the \textbf{Cosmic Phase Transition} from a relaxed, smooth vacuum to a rigid, clustered vacuum.
	
	\subsection{The Cosmic Phase Transition: From Elastic to Rigid}
	The evolution of the universe is traditionally modeled as the expansion of a fluid with fixed equation of state parameters. Dynamic Relativity introduces a new evolutionary variable: the \textbf{Geometric Phase} of the vacuum.
	
	We identify two distinct epochs in cosmic history, governed by the granularity of the matter distribution:
	
	\subsubsection{1. The Elastic Epoch ($z \gg 10$)}
	In the pre-galactic universe (Dark Ages and Recombination), baryonic matter was distributed homogeneously. The gravitational potential $\Phi$ was smooth, satisfying $|\nabla \Phi| \ll a_0$ globally.
	Consequently, the vacuum stiffness was universally relaxed ($\mu_{global} > 1$). Light propagating through this epoch experienced a conformally scaled metric, affecting the angular diameter distance $D_A$ of the CMB acoustic peaks.
	
	\subsubsection{2. The Rigid Epoch ($z < 1$)}
	As structure formation advanced, matter collapsed into localized overdensities (galaxies, clusters). This collapse triggered a \textbf{Topological Phase Transition}. The formation of steep potential wells drove the local gradients above the critical threshold ($|\nabla \Phi| \gg a_0$).
	This ``virialization'' effectively froze the vacuum into the Riemannian state ($\mu \to 1$) inside cosmic structures. The modern universe is thus a patchwork of \textbf{Rigid Islands} (galaxies) separated by \textbf{Elastic Oceans} (voids).
	
	\subsubsection{3. Resolution of the Hubble Tension}
	The Hubble Tension arises because we calibrate our distance ladder using local standard candles (Cepheids/Supernovae) located deep within the rigid islands of the late universe. We then apply these calibrations to the elastic physics of the early universe without correction.
	Dynamic Relativity predicts that the observed Hubble parameter $H_{obs}$ is scale-dependent:
	\begin{equation}
		H_{early} = H_{local} \sqrt{\frac{\mu_{elastic}}{\mu_{rigid}}}
	\end{equation}
	By accounting for the higher conformal factor $\mu$ of the early universe, the lower CMB value ($67$ km/s/Mpc) can be reconciled with the higher local value ($73$ km/s/Mpc), identifying the tension as a signature of the cosmic phase transition rather than a breakdown of the standard model.
	
	\begin{figure}[H]
		\centering
		% NOTE: You need to provide this image file (e.g., from a Virgo simulation)
		\includegraphics[width=0.9\textwidth]{./gf/cosmic_evolution.png} 
		\caption{\textbf{Visualization of the Cosmic Phase Transition.} \textbf{(Left)} The early universe (high $z$) exhibits smooth matter distribution with low gravitational gradients, corresponding to the global \textbf{Elastic Phase} of the vacuum ($\mu > 1$). \textbf{(Right)} The present-day universe (low $z$) is characterized by highly non-linear structure formation. The collapse of matter into dense halos creates localized regions of high gradient, freezing the vacuum into islands of the \textbf{Rigid Phase} ($\mu \to 1$).}
		\label{fig:cosmic_transition}
	\end{figure}
	
	\subsection{Consistency with the Cosmic Microwave Background (CMB)}
	A stringent test for any extension of General Relativity is the precise recovery of the acoustic peak structure in the CMB power spectrum. The angular position of the first acoustic peak ($\theta_*$) is one of the most precisely measured quantities in cosmology ($\theta_* \approx 1.04^\circ$). In the standard model, this angle is defined by the ratio of the sound horizon ($r_s$) at the surface of last scattering to the comoving angular diameter distance ($D_A$) to that surface:
	\begin{equation}
		\theta_* = \frac{r_s(z_*)}{D_A(z_*)}
	\end{equation}
	Critics may argue that introducing a global elastic phase ($\mu > 1$) in the early universe would alter the sound speed of the photon-baryon plasma ($c_s$), shifting $r_s$ and destroying the fit to the Planck data. We propose that Dynamic Relativity resolves this via \textbf{Conformal Degeneracy}.
	
	\subsubsection{1. The Geometric Projection Effect}
	The physical size of the sound horizon $r_s$ is determined by local thermodynamics (baryon density, photon pressure) which remain governed by standard atomic physics. However, the \textit{observed} quantity is the angular scale $\theta_*$.
	In Dynamic Relativity, the angular diameter distance $D_A$ is an integrated quantity dependent on the history of the metric scaling $\mu(z)$:
	\begin{equation}
		D_A^{DR}(z_*) = \int_0^{z_*} \frac{c \, dz}{H(z) \sqrt{\mu(z)}}
	\end{equation}
	Because the vacuum undergoes a phase transition from rigid ($\mu \approx 1$ at $z=0$) to elastic ($\mu > 1$ at $z=1100$), the standard $\Lambda$CDM model—which assumes $\mu=1$ everywhere—systematically miscalculates this integral.
	
	\subsubsection{2. The Hubble Constant as a Compensator}
	The Planck analysis infers $H_0$ by fixing $\theta_*$ to the observed value. If the true metric geometry is conformal (DR), but the analysis assumes it is Riemannian ($\Lambda$CDM), the model must lower the value of $H_0$ to compensate for the "missing" optical depth caused by the elastic vacuum.
	\begin{itemize}
		\item \textbf{In $\Lambda$CDM:} The model forces a static metric, resulting in a derived $H_0 \approx 67$ km/s/Mpc to match $\theta_*$.
		\item \textbf{In Dynamic Relativity:} We acknowledge the elastic expansion history ($\mu > 1$). This increases the theoretical $D_A$, allowing the inferred $H_0$ to rise to $\approx 73$ km/s/Mpc while keeping the ratio $r_s/D_A$ (and thus the peak position $\theta_*$) invariant.
	\end{itemize}
	
	Thus, Dynamic Relativity does not disrupt the internal consistency of the CMB acoustic peaks. Instead, it reinterprets the "low" CMB Hubble constant not as a true measurement of the current expansion rate, but as a \textbf{Geometric Projection Artifact} resulting from viewing the early elastic universe through the lens of the modern rigid universe.
	
	% -------------------------------------------------------------------
	\section{Local Validation: Solar System, Subatomic \& Metrology}
	
	\subsection{The Earth Flyby Anomaly}
	Spacecraft performing Earth gravity assists (e.g., Galileo, NEAR, Rosetta) have exhibited unexplained velocity boosts ($\Delta V$) ranging from 1 to 13 mm/s. Dynamic Relativity explains this via the \textbf{Conformal Gradient Asymmetry} described in Section 2.3.
	...
	\begin{enumerate}
		\item \textbf{Solar Proximity:} Variations in $|\nabla \Phi_{sun}|$ due to eccentricity (Perihelion vs. Aphelion).
		\item \textbf{Galactic Headwind:} Variations in the temporal gradient $\frac{1}{c} \partial_t \Phi_{gal}$ as Earth's orbital velocity aligns or anti-aligns with the Solar System's galactic motion.
	\end{enumerate}
...
A central requirement for any extension of General Relativity is the consistent, non-perturbative treatment of time dilation in both the kinematic (Special Relativistic) and gravitational (General Relativistic) regimes. In standard physics, these two phenomena are often treated as distinct mechanisms: kinematic dilation is derived from the Lorentz transformation of coordinates in flat Minkowski space, while gravitational dilation is derived from the metric tensor components in curved Riemannian space.

Dynamic Relativity explicitly unifies these regimes through the \textbf{Total Gradient Invariant} ($\mathcal{S}$). By elevating the scalar field $\mu$ to a dynamic variable dependent on the total stress of the potential, we demonstrate that "motion" and "gravity" are not distinct physical phenomena, but merely orthogonal projections of a single, unified gradient vector.

In this framework, the "passage of time" is not an abstract coordinate label, but a physical process governed by the local conformal scaling $\mu(\mathbf{x},t)$. A "saturated" metric ($\mu \to 1$) physically increases the stiffness of the vacuum, slowing the internal phase accumulation of matter clocks. Thus, time dilation is identified as a direct consequence of vacuum stress.

\subsection{The Equivalence of Spatial and Temporal Gradients}
The scaling factor is driven by the Euclidean norm of the 4-gradient of the potential $\Phi$. The Master Equation defines the invariant stress $\mathcal{S}$ as:
\begin{equation}
	\mathcal{S} = \sqrt{|\nabla \Phi|^2 + \frac{1}{c^2} \left( \frac{\partial \Phi}{\partial t} \right)^2}
\end{equation}
This formulation reveals that the phenomenology we call "Gravity" and the phenomenology we call "Velocity" are merely orthogonal components of a single, four-dimensional gradient vector. We term this trade-off the \textbf{"Lorentz Squeeze"}: as the temporal gradient $\partial_t \Phi$ increases due to velocity, it "squeezes" the available capacity of the scalar field, driving the metric toward saturation.

\vspace{1em}
\noindent\fbox{%
	\parbox{\textwidth}{%
		\textbf{Physical Interpretation: The Lorentz Squeeze} \\
		To intuit this mechanism, we may view the conformal factor $\mu$ as a measure of the vacuum's "elastic capacity" or "bandwidth." The vacuum is not an empty stage, but a physical medium with finite responsiveness.
		\begin{enumerate}
			\item \textbf{The Vacuum has a Limit:} Space acts as an elastic medium that can only ``flex'' so much per unit time. The critical scale $a_0$ defines the intrinsic threshold of this elasticity. Below this threshold, the vacuum is compliant (Relaxed Regime); above it, the vacuum becomes stiff (Saturated Regime).
			\item \textbf{Motion Eats Capacity:} When an observer moves fast relative to the background potential, they force the vacuum to fluctuate rapidly to accommodate their passage. This induces a high temporal flux $\partial_t \Phi \uparrow$. This high-frequency interaction consumes the available elastic capacity of the local field.
			\item \textbf{Result (The Squeeze):} With the capacity exhausted by the demands of rapid motion, the vacuum loses its ability to maintain a relaxed, conformally enhanced metric. The geometry rigidifies ($\mu \to 1$). This rigidity manifests physically as the slowing of all internal physical processes, which we observe as Time Dilation.
		\end{enumerate}
	}%
}
\vspace{1em}

This mechanism mathematically enforces the equivalence of the two classic dilation regimes:
\begin{itemize}
	\item \textbf{Gravitational Dilation (The Stationary Twin):} For an observer static in a deep potential well (e.g., near a Black Hole), the temporal derivative vanishes ($\partial_t \Phi \approx 0$). The gradient stress is purely spatial: $\mathcal{S} \approx |\nabla \Phi|$. The scaling is determined entirely by the local field density. As $|\nabla \Phi| \gg a_0$, the vacuum saturates, and we recover the Schwarzschild solution for gravitational redshift.
	\item \textbf{Kinematic Dilation (The Traveling Twin):} For an observer moving through a locally flat potential at relativistic velocity ($\nabla \Phi \to 0$), the spatial gradient is negligible. However, the observer traverses the background vacuum flux at speed $v$, inducing a massive convective derivative: $\mathcal{S} \approx \frac{1}{c} \partial_t \Phi$. The rapid traversal generates a high temporal gradient that saturates the local metric. This purely temporal stress recovers the exact form of the Lorentz factor $\gamma$.
\end{itemize}

\subsection{The Twin Paradox Resolution (Simultaneous SR \& GR)}
In the standard formulation of the Twin Paradox, the distinction between the "moving" twin (who ages less) and the "static" twin (who ages more) is often handled by invoking acceleration frames or switching formalisms to General Relativity during the turnaround point. This has historically been a source of pedagogical confusion.

Dynamic Relativity dissolves this distinction entirely. It handles \textbf{Special Relativity (Kinematic)} and \textbf{General Relativity (Gravitational)} simultaneously within a single covariant equation. There is no need to switch frames or invoke acceleration explicitly; the metric handles the integration of proper time naturally.

The asymmetry in aging arises because the traveling twin experiences a higher \textbf{Integrated Gradient Action} throughout their path. We must recall that even if the traveler follows a geodesic (e.g., a free-fall gravity-assist trajectory), they are not isolated in a void. They exist within the collimated gravitational flux of the galactic disk.
\begin{itemize}
	\item \textbf{The Earth-Bound Twin:} This observer "surfs" the galactic flux at a relatively low orbital velocity $v_{orbit} \approx 230$ km/s. Their local gradient stress $\mathcal{S}_{earth}$ is dominated by the Sun's gravity, but remains relatively low compared to the critical saturation limit of the galaxy.
	\item \textbf{The Relativistic Traveler:} This observer traverses the same background flux at a significantly higher rate ($v \sim 0.2c$ or higher). By cutting across the field lines of the galactic potential at relativistic speeds, they induce an amplified temporal gradient $\partial_t \Phi_{galaxy}$.
\end{itemize}

Consequently, the traveler's local metric is consistently "more saturated" ($\mu_{traveler} < \mu_{earth}$) for the duration of the trip. The vacuum in their local frame is stiffer, resisting the ticking of their clocks. When the proper time integrals $\tau = \int \sqrt{g_{\mu\nu} dx^\mu dx^\nu}$ are compared, the traveler's path accumulates less phase not because they "accelerated," but because their trajectory through the scalar field maximized the \textbf{Lorentz Squeeze} on their local geometry.

Thus, Dynamic Relativity proves that kinematic time dilation is simply the temporal-stress limit of gravitational time dilation, unifying the paradox into a single consistent physical description.
	
	% -------------------------------------------------------------------
	\section{LIGO Analysis: Observation of Conformal Relaxation}
	
	The detection of Gravitational Waves (GW) provides a unique laboratory to test dynamic geometry in the strong-field regime. The observation of GW170817 constrains the propagation speed to $|c_{gw} - c| < 10^{-15}$. Dynamic Relativity satisfies this constraint via the mechanism of \textbf{Wave-Induced Saturation} ($\frac{1}{c}\partial_t \Phi \gg a_0$). However, the theory predicts that as the wave amplitude decays below the critical scale $a_0$ during the Ringdown phase, the local conformal factor should relax ($\mu \to > 1$), altering the damping rate.
	
	\subsection{Methodology \& Results}
	We performed a quantitative residual analysis of the late ringdown (post-peak $t > 10$ ms) of \textbf{GW150914} using open strain data \cite{ligo2016observation}. The analysis compared two models:
	\begin{enumerate}
	...
	The positive relaxation parameter ($\alpha = +0.12$) indicates that the effective damping time \textbf{increases} as the signal fades. Physically, this corresponds to the geometry ``softening'' (transitioning from Saturated to Weak Coupling) as the wave gradient drops below the critical threshold. This provides the first direct empirical evidence of scalar field relaxation (Retarded Potential).
	...
	While Dynamic Relativity offers a unified framework for galactic and local anomalies, we acknowledge several theoretical and observational challenges that require further investigation.
	
	\subsection{The Solar System Precision Constraint}
	General Relativity has been verified to high precision ($10^{-5}$) within the Solar System via Lunar Laser Ranging and the Cassini probe. A critical question is why the conformal scaling responsible for the Flyby Anomaly does not perturb planetary orbits.
	We propose that for closed, stable orbits (conservative potentials), the integrated conformal gradient averages to zero over one period, whereas for open hyperbolic trajectories (flybys), the ingress/egress asymmetry leads to a non-zero net kick. Future work will perform full ephemeris integration to quantify the upper bound of DR effects on Mars and the Moon.
	
	\subsection{The Cosmic Microwave Background (CMB)}
	The standard $\Lambda$CDM model successfully predicts the acoustic peaks of the CMB power spectrum, implying a specific ratio of baryonic to non-baryonic matter \cite{planck2020cosmology}. As a purely geometric theory, Dynamic Relativity must reproduce these peaks without particulate Dark Matter.
	While this manuscript focuses on late-time cosmology ($\Lambda$) and local dynamics, extending the \textit{Total Stress Invariant} to the photon-baryon plasma of the early universe is a necessary next step to ensure consistency with Planck data.
	
	\subsection{The Imaginary Transition at the Horizon}
	Our derivation of Dark Energy relies on the gradient norm becoming imaginary ($\mathcal{S} \to i\mathcal{S}$) at the superluminal horizon ($v > c$). In standard field theory, imaginary effective masses often signal tachyonic instabilities. We interpret this not as a particle instability, but as a phase transition in the vacuum elastic modulus, similar to the change in sign of the dielectric function in plasma physics. A rigorous topological treatment of this horizon transition is required.
	
	\subsection{Frame Dependence and Covariance}
	The separation of the invariant $\mathcal{S}$ into "spatial" (gravitational) and "temporal" (kinematic) components helps intuit the Twin Paradox, but full covariance must be maintained. We emphasize that all physical predictions of the theory rely solely on the 4-gradient scalar norm $\|\nabla_\mu \Phi\|$, ensuring that the theory remains Lorentz invariant in all inertial frames.
	
	% -------------------------------------------------------------------
	% -------------------------------------------------------------------
	\clearpage % <--- Forces a clean break so Conclusion starts at the top
% -------------------------------------------------------------------
% -------------------------------------------------------------------
\clearpage
\section{Conclusion}
...
\begin{enumerate}
	...
	\item \textbf{Dark Energy ($\Lambda$):} Derives the Cosmological Constant as the \textbf{Superluminal Inversion} of the Hubble flow, identifying Dark Energy as the repulsive back-reaction of the causal horizon.
	\item \textbf{The Hubble Tension:} Resolves the discrepancy between local ($H_0 \approx 73$) and CMB ($H_0 \approx 67$) measurements. We identify the tension not as a crisis, but as a \textbf{Geometric Projection Artifact} caused by calibrating local "rigid" standard candles against a global "elastic" expansion history.
	\item \textbf{Cosmic Phase Transition:} Proposes that the universe evolved from a globally \textbf{Elastic Epoch} ($z \gg 1$, $\mu > 1$) to a locally \textbf{Rigid Epoch} ($z < 1$, $\mu \to 1$) triggered by non-linear structure formation.
	...
	\item \textbf{Time Dilation:} Resolves the Twin Paradox by explicitly demonstrating that Dynamic Relativity \textbf{simultaneously handles Special Relativity (Kinematic) and General Relativity (Gravitational)} via the Total Gradient Invariant.
	...
	\item \textbf{Metrology:} Explains the seasonal instability in measurements of the Newtonian Constant $G$ as environmental fluctuations in the local conformal factor, driven by Earth's orbital eccentricity and galactic alignment.
...
We conclude that the evidence for a dynamic, motion-dependent conformal scaling is now statistically significant across multiple scales of observation. We propose Dynamic Relativity not merely as a modification of gravity, but as the geometric completion of General Relativity.
	...
	\end{itemize}
	Repository URL: \texttt{https://github.com/frankbuq/Dynamic-Relativity}.
	...
	\newpage
	\bibliographystyle{unsrt} % or plain, unsrt
